...
const About = () => {
  ...
  useEffect(() => {
    // Infinite rotation for first set of elements (divRefs1)
    divRefs1.current.forEach((ref, index) => {
      if (!ref) return; // Ensure ref exists
      const rotationDirection = index % 2 === 0 ? 360 : -360; // Alternate direction for even/odd
      gsap.to(ref, {
        rotation: rotationDirection,
        duration: 26,
        repeat: -1,
    ...
    // Infinite rotation for second set of elements (divRefs2)
    divRefs2.current.forEach((ref, index) => {
      if (!ref) return;
      ...
      const moveElements = (refs) => {
        refs.current.forEach((ref, index) => {
          ...
          gsap.to(ref, {
            x: movement.x,
            y: movement.y,
            duration: 1 + (index % 4) * 0.2,
            ease: "power3.out",
          });
        });
      };

      moveElements(divRefs1);
      moveElements(divRefs2);
    };

    // Reset on mouse leave
    const handleMouseLeave = () => {
      const resetElements = (refs) => {
        refs.current.forEach((ref, index) => {
          if (!ref) return;
          gsap.to(ref, {
            x: 0,
      ...
      resetElements(divRefs1);
      resetElements(divRefs2);
    };
